...
def glitches(setting):
    ...
    out = ROOT / "results" / "scratch_glitch" / CLASS_BUILD / setting.replace(" ", "_").replace("+", "and")
    ...
    return {"setting": setting, "class_build": CLASS_BUILD, "extra_precision": SETTINGS[setting], "n_models": len(rows),
...
def main():
    ...
    # stock results keep their original keys; other builds are suffixed with the build name
    old.update({r["setting"] + ("" if CLASS_BUILD == "stock" else f" [{CLASS_BUILD}]"): r for r in results})
