...
\author{An explainer for the \texttt{class\_baryons} project}
...
If this estimate is correct, it changes the character of the problem. Extra baryons can avoid loading the photon--baryon fluid only if they are clumped before recombination, and the drag-based constraints of the previous sections assume clumps that exist, with the stated surface density, at redshifts of $4\times10^4$ (for the CMB) or $10^7$ (for small-scale structure). Planetary-mass clumps of ordinary density could not have been bound at those epochs, while clumps that were bound would have to be of stellar mass or larger, or of degenerate density, and stellar-mass clumps are subject to other constraints that lie outside this project.

\section{Summary and outlook}
\label{sec:summary}

The calculations described here establish four results. The CMB is sensitive to a population of photon-coupled clumps through the drag that radiation exerts on them in the radiation era, which suppresses their clustering and changes the gravitational potentials seen by the photons; the photons themselves barely scatter off the clumps at CMB-relevant couplings. Because the drag depends only on the momentum-transfer cross-section, the constraint derived for an elementary scatterer applies to opaque macroscopic clumps, and the published Planck analysis implies a minimum surface density of about $10^6\,\mathrm{g\,cm^{-2}}$. The same drag produces a cutoff in the matter power spectrum that, when compared with modern warm dark matter limits, raises the minimum surface density to roughly $5\times10^{10}\,\mathrm{g\,cm^{-2}}$, provided the clumps existed by $z\sim10^7$. Finally, because the clumps are held at the radiation temperature, planetary-mass clumps of ordinary density cannot remain gravitationally bound at the epochs these constraints probe, and spectral distortions provide no useful limit on passive clumps.

The next stage of the project will therefore focus on formation history: mapping the combinations of clump mass, density and formation redshift that can survive, determining when the extra baryons must be removed from the photon--baryon fluid to leave the CMB acoustic peaks unchanged, and asking whether any combination satisfies both requirements. The formal CMB likelihood analysis, which will reproduce the published Planck bound and extend it to populations in which only part of the dark matter is clumped, remains to be done, but its importance will depend on the answer to the survival question.
