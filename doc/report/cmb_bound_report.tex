...
\begin{abstract}
We report the first real-data constraint from this project on the momentum coupling between the cosmic microwave background (CMB) photons and a population of compact, pressureless clumps that make up all of the dark matter. The coupling is expressed through the dimensionless interaction strength $u$ used by the \textsc{class} Boltzmann code, which maps directly onto the clump cross-section per unit mass and hence onto a minimum clump surface density. Profiling the likelihood over the six standard cosmological parameters and all instrumental and foreground nuisance parameters, the Planck 2018 temperature and polarization data give $u < 1.00\times10^{-4}$ at 95\% confidence, equivalent to $\sigma/M < 3.7\times10^{-7}\,\mathrm{cm^2\,g^{-1}}$ and a clump surface density $\Sigma/Q > 2.7\times10^{6}\,\mathrm{g\,cm^{-2}}$, where $Q$ is an order-unity momentum-transfer efficiency. The independent Planck PR4 (NPIPE) CamSpec high-$\ell$ likelihood gives a limit 17\% weaker, $u < 1.17\times10^{-4}$. At zero coupling the pipeline reproduces Planck's own $\Lambda$CDM fit, and the limits agree with a Fisher forecast made before any data were used. These numbers are conditional on a specific and deliberately narrow model, in which the clumps are added to an otherwise standard $\Lambda$CDM universe with homogeneous recombination, a smooth diffuse baryon fluid and adiabatic initial conditions. Physics that would plausibly accompany the formation of such clumps, such as small-scale inhomogeneity in the remaining gas or primordial magnetic fields, is not included and could move the limit in either direction. A Bayesian analysis with a flat prior on $u$ gives $u < 1.69\times10^{-4}$ ($\Sigma/Q > 1.6\times10^{6}\,\mathrm{g\,cm^{-2}}$), close to published Bayesian limits. The difference from the profile limit follows from the nearly linear rise of the profile. The coupling is strongly anticorrelated with $\sigma_8$, which shows that the limit comes from the suppression of structure seen through CMB lensing.
\end{abstract}
...
It is worth being explicit about what kind of statement the answer is. A likelihood analysis with \textsc{class} does not ask which universes could produce the observed sky; it asks which parameter values within one family of dynamical models do so. The family used here is $\Lambda$CDM, with its homogeneous recombination, linear perturbations of a smooth baryon fluid, standard atomic and neutrino physics, and adiabatic, nearly scale-invariant initial conditions, extended by a single new ingredient: a pressureless component that feels a drag force from the radiation. The limit on $u$ is therefore a statement of the form ``if the clumps are the only departure from $\Lambda$CDM, a coupling larger than this worsens the fit to Planck by more than $\dchi = 2.71$''. It is not a model-independent measurement of the opacity of baryonic clumps. Section~\ref{sec:scope} discusses which of the fixed assumptions a real clump population might violate, and what is known about how relaxing them changes the CMB.
...
\midrule
PR4 CamSpec (MCMC) & $1.69\times10^{-4}$ & -- & $<6.3\times10^{-7}$ & $>1.6\times10^{6}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Scope of the result: what the model leaves out}
\label{sec:scope}

The robustness checks above test the calculation within its model. A separate question is how much the answer depends on the model itself, and here the honest position is that the limit applies to a highly specialised scenario. The clumps are inserted into an otherwise standard universe, and everything else about the baryons, recombination and the initial conditions is held at its $\Lambda$CDM form. If compact baryonic clumps really made up the dark matter, they would have to form from large, early departures from homogeneity in the baryons, and it is unlikely that such a process would leave the rest of the baryonic physics untouched. The more useful question for the next stage is therefore not which arbitrary extra parameters could loosen the bound, but what additional early-universe physics necessarily accompanies the existence of the clumps, and how that physics changes the CMB.

\begin{table}[t]
\centering
\caption{Assumptions fixed in the present calculation, and how a population of early compact baryonic clumps might violate them.}
\label{tab:assumptions}
\begin{tabular}{p{0.2\textwidth}p{0.3\textwidth}p{0.4\textwidth}}
\toprule
Ingredient & Present model & Possible departure in a clump cosmology \\
\midrule
Diffuse baryons & smooth fluid, linear perturbations & residual small-scale clumping of the gas that did not end up in compact objects \\
Recombination & homogeneous, $x_e(z)$ from the mean density & density-weighted, $\langle x_e(\rho_b)\rangle$, altered by clumping and by Lyman-$\alpha$ transport between regions \\
Magnetic fields & none & primordial fields as a formation seed, with Lorentz forces and stresses \\
Initial conditions & adiabatic, power law & extra small-scale power or isocurvature modes from the seeding process \\
Clumps & pressureless, point-like, fixed $\sigma/M$, present throughout & mass and size distributions, formation at a finite redshift, evolution of $\sigma/M$ \\
Baryon budget & $\omega_b$ from the CMB equals the BBN value & compact baryons are counted as dark matter by the CMB, in tension with BBN unless formed very early \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:assumptions} lists the main candidates. The first two rows have been studied in depth for reasons unrelated to dark matter. Recombination depends on the square of the density, so unresolved small-scale inhomogeneity in the baryons changes the mean ionization history even when it is invisible as a resolved perturbation. \citet{JedamzikSaveliev2019} showed that primordial magnetic fields generate exactly this kind of clumping and used it to limit the field strength. Phenomenological descriptions of clumping, with a few zones of different density, have been fitted to the CMB \citep{Rashkovetskyi2021,Thiele2021}, and the effects are large enough to be tested. More recently, magnetohydrodynamic simulations coupled to recombination and Lyman-$\alpha$ radiative transfer have derived the ionization history from the field statistics instead of assuming a clumping law \citep{Jedamzik2023}, and fits of the resulting model to Planck, baryon acoustic oscillation and supernova data are at least as good as those of $\Lambda$CDM while favouring a different region of parameter space \citep{Jedamzik2025}. These studies show that richer, physically coupled small-scale baryon physics can change which cosmologies fit Planck, and they also show the route by which such physics can be brought into a precision analysis. The small-scale calculation produces effective quantities such as the ionization history, and these are fed into the Boltzmann code, which is then compared with the likelihood.

Two points limit how far this freedom extends. First, the Planck data are heavily overdetermined, with temperature and polarization spectra over thousands of multipoles as well as lensing, and modifications that repair one feature commonly spoil the damping tail, the polarization phases or the matter power spectrum elsewhere; the clumping studies above found exactly this kind of restriction. Second, the coupling studied here is seen mainly as a suppression of small-scale growth and hence of lensing. Accompanying physics can only relax the limit by adding back lensing or small-scale growth, or by moving the standard parameters in a way that compensates. A modified recombination history does not add lensing, but it does change the damping tail and the sound horizon, and the Fisher analysis found the coupling to be most degenerate with $\theta_s$ and $\omega_b$, so an interaction between the two effects through the standard parameters cannot be ruled out. Whether the net effect of a self-consistent clump cosmology is to loosen or to tighten the limit is unknown, and cannot be established without a model of how the clumps form.

For these reasons we regard the present result as the first layer of the analysis: a clean, conditional limit in which a single effect is isolated. A second layer would audit the rows of Table~\ref{tab:assumptions} quantitatively for a physically specified formation scenario, and a third would derive the effective inputs to the Boltzmann calculation from small-scale simulations of that scenario, in the manner of the magnetic-field work.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{report_mcmc.pdf}
\caption{Markov chain Monte Carlo results for PR4 CamSpec with the 2018 low-$\ell$ likelihoods, $f_\cl = 1$ and a flat prior on $u$. Top left: the marginalised posterior of $u$, with the MCMC and profile 95\% limits and the published 2018 limit of \citet{Zhou2022}. Other panels: posterior samples of $u$ against the parameters it is most correlated with, with the Pearson correlation coefficient $r$ and the MCMC 95\% limit (dashed).}
\label{fig:mcmc}
\end{figure}

We sampled the posterior with the Metropolis sampler in Cobaya, using the PR4 CamSpec and 2018 low-$\ell$ likelihoods. The coupling was given a flat prior on $[0, 10^{-3}]$. \citet{DiacoumisWong2019} showed that such a prior gives limits consistent to within 20\% with the Jeffreys prior, whereas a prior flat in $\log u$ makes the limit depend on an arbitrary lower cut-off. The six cosmological and nine nuisance parameters were sampled alongside $u$ in 24 independent chains, started near the profile best fit with a proposal distribution taken from the covariance measured in the profile at $u=0$. The sampler exploits the fact that the nuisance parameters can be varied without recomputing the CMB spectra. After removing the first 30\% of each chain, the combined chains contain about 30\,000 accepted samples. The Gelman--Rubin statistic for the mean of $u$ across chains is $R-1 = 0.02$, and the 95\% limit computed from the first and second halves of every chain agrees to better than 1\% ($1.69$ and $1.69\times10^{-4}$). The marginalised 95\% and 99\% upper limits are
\begin{equation}
u < 1.69\times10^{-4}\ (95\%), \qquad u < 2.33\times10^{-4}\ (99\%),
\end{equation}
corresponding to $\sigma/M < 6.3\times10^{-7}\,\mathrm{cm^2\,g^{-1}}$ and $\Sigma/Q > 1.6\times10^{6}\,\mathrm{g\,cm^{-2}}$ at 95\%. The posterior of $u$ peaks at zero and falls monotonically (Figure~\ref{fig:mcmc}), with no hint of a preference for a non-zero coupling. The standard parameters are close to their $\Lambda$CDM values, with $H_0 = 67.1 \pm 0.5\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$ and $n_s = 0.962 \pm 0.004$, and $\sigma_8 = 0.78 \pm 0.02$ is lower than in $\Lambda$CDM because it is dragged down by the coupling.

The Bayesian limit is 45\% weaker than the profile limit from the same likelihood. The difference is what one expects from the shape of the profile. Because $\dchi$ rises almost linearly from $u=0$, the likelihood falls roughly exponentially in $u$, and for an exponential the value that encloses 95\% of the probability lies about twice as far out as the point where $\dchi = 2.71$. For a quadratic $\dchi$ the ratio would be only 1.2. Integrating the profile likelihood itself, $\exp(-\dchi/2)$, over the same flat prior gives $u < 1.96\times10^{-4}$. The full MCMC limit lies between that and the profile crossing, so marginalising over the other parameters slightly favours small couplings over what the profile alone would suggest, and there is no sign of a volume effect that inflates the Bayesian limit. The MCMC limit is close to the published Bayesian limits, $1.55\times10^{-4}$ from the 2018 data \citep{Zhou2022} and $1.58\times10^{-4}$ from the 2015 data \citep{StadlerBoehm2018}, which confirms that most of the apparent disagreement between our profile limit and the literature was the difference between the two kinds of statistic.

The correlations in Figure~\ref{fig:mcmc} show how the data see the coupling. The strongest is with $\sigma_8$ ($r = -0.92$): the clumps are dragged by the radiation and their small-scale perturbations are suppressed, so the amplitude of matter fluctuations today falls as $u$ grows. It is this reduction in structure, seen by Planck through the lensing of the CMB, that limits $u$. The second is a positive correlation with the angular sound horizon $\theta_s$ ($r = +0.73$). The weaker potentials probably shift the phases of the acoustic peaks slightly, and the fit absorbs the shift by moving $\theta_s$. That would agree with the degeneracy with $\theta_s$ found in the Fisher analysis. The correlations with $\omega_{\rm dm}$ and $\omega_b$ are weaker ($r = +0.28$ and $+0.16$), and $H_0$ is essentially uncorrelated with $u$, so the coupling does not help with the Hubble tension. The strong anticorrelation with $\sigma_8$ also shows that the constraint rests on the amplitude of lensing, which is why the treatment of the Planck lensing excess and the addition of the lensing reconstruction matter so much to the precise value of the limit.

\section{Summary}

If all of the dark matter were in compact clumps coupled to the photons through a geometric cross-section, and nothing else departed from $\Lambda$CDM, the Planck 2018 temperature and polarization spectra require the coupling to satisfy $u < 1.0\times10^{-4}$ at 95\% confidence, or equivalently a clump surface density $\Sigma/Q > 2.7\times10^{6}\,\mathrm{g\,cm^{-2}}$. The final Planck processing gives a limit 17\% weaker, and the difference between the two likelihoods is consistent with the known difference in their preference for extra lensing. The limit agrees with an advance forecast and is tighter than the published marginalised limits from the 2015 and 2018 data, as a profile limit on a parameter with a physical boundary is expected to be. The application of the particle bound to macroscopic dark matter was first made by \citet{Jacobs2015}; what this analysis adds is a dedicated treatment of opaque clumps with current likelihoods. The limit is conditional on that narrow model, and physics that plausibly accompanies clump formation, notably small-scale inhomogeneity of the remaining gas and its effect on recombination, is not included. A Bayesian analysis with the PR4 likelihood gives $u < 1.69\times10^{-4}$, or $\Sigma/Q > 1.6\times10^{6}\,\mathrm{g\,cm^{-2}}$. The difference from the profile limit follows from the nearly linear shape of the profile, and brings the result into line with the published Bayesian limits. The main open items are a test of how much the Planck lensing excess tightens the bound, the extension to populations in which only part of the dark matter is clumped, and an audit of which standard assumptions a specific formation scenario would violate.
...
\bibitem[Jedamzik et~al.(2023)]{Jedamzik2023}
Jedamzik K., Abel T., Ali-Ha\"imoud Y., 2023, \emph{Cosmic recombination in the presence of primordial magnetic fields}, arXiv:2312.11448.

\bibitem[Jedamzik et~al.(2025)]{Jedamzik2025}
Jedamzik K., Pogosian L., Abel T., 2025, \emph{Hints of primordial magnetic fields at recombination and implications for the Hubble tension}, Nature Astronomy, arXiv:2503.09599.
...
\bibitem[Rashkovetskyi et~al.(2021)]{Rashkovetskyi2021}
Rashkovetskyi M., Mu\~noz J.~B., Eisenstein D.~J., Dvorkin C., 2021, \emph{Small-scale clumping at recombination and the Hubble tension}, Phys.\ Rev.\ D 104, 103517.
...
\bibitem[Thiele et~al.(2021)]{Thiele2021}
Thiele L., Guan Y., Hill J.~C., Kosowsky A., Spergel D.~N., 2021, \emph{Can small-scale baryon inhomogeneities resolve the Hubble tension? An investigation with ACT DR4}, Phys.\ Rev.\ D 104, 063535.
