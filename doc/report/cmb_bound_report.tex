...
The published bounds come from \citet{StadlerBoehm2018}, who analysed the 2015 Planck release and found $u < 1.58\times10^{-4}$ from temperature and polarization and $u < 2.25\times10^{-4}$ from temperature alone. Our limit is about 35\% tighter than the first of these. The difference is expected in direction and plausible in size, because the 2018 release has substantially better polarization data and a revised treatment of the large-scale polarization, and because their limits are Bayesian marginalised limits while ours come from a profile, and the two need not coincide when the likelihood is not Gaussian in $u$. The Bayesian analysis described in Section~\ref{sec:mcmc} will allow a like-for-like comparison.
